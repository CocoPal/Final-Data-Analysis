...
country_plot <- ggplot(merged_data, aes(x = Year)) +
  geom_col(aes(y = N, fill="redfill")) +
  geom_text(aes(y = N, label = N), fontface = "bold", vjust = -0.2, color = "black", size = 2) +
...
#2. Overcoming the issue with needing two y-axis
all_country$IDPs_total <- all_country$IDPs_total/100000

#3. Creating the plot 
ggplot(all_country, aes(x = Year)) +
  ...
  scale_fill_manual('', labels = 'The number of natural disester event', values = "#FF6666") +
  scale_color_manual('', labels = 'The number of IDPs*100 000', values = 'black') +
  theme_bw() +
  theme(axis.text.x=element_text(angle=90, hjust=1))
