...
# Plot 
ggplot() + 
  geom_bar(aes(x = merged_data$Year, y = merged_data$N), stat = "identity", fill = "grey") +
  geom_line(aes(x = merged_data$Year, y = merged_data$IDPs), size = 2, color = "blue") + 
  scale_y_continuous(name = "the number of events", 
     sec.axis = sec_axis(~., name = "the number of IDPs"))

dual <- ggplot(merged_data, aes(x = Year))
dual <- dual + geom_bar(aes(y = N), stat="identity", position="dodge") +  facet_wrap(~Country, ncol=2) +  
  theme_bw() +
  theme(axis.text.x=element_text(angle=90, hjust=1))

dual <- dual + geom_line(aes(y = IDPs), color = "red", size = 2)
