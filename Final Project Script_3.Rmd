...
Cam_Lao_V_Tai <- merged_data %>%
  ...
  group_by(Year, Country) %>%
  summarise(IDPs_total = sum(IDPs), events_total = sum(N))

Cam_Lao_V_Tai$IDPs_total <- Cam_Lao_V_Tai$IDPs_total/100000

ggplot(Cam_Lao_V_Tai, aes(x = Year)) +
  ...
  theme_bw() +  
  facet_wrap(~Country, ncol=2) +
  theme(axis.text.x=element_text(angle=90, hjust=1))ммммм
...
all_plot <- ggplot(all_country, aes(x = Year)) +
  ...
  labs(x = "Year", y = "") +
  ggtitle("The number of natural disaster events vs the number of IDPs in Southeast Asia") +
  scale_fill_manual('', labels = 'The number of natural disester event', values = "#009999") +
  scale_color_manual('', labels = 'The number of IDPs*100 000', values = "#FF6633") +
  theme_bw() +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  theme(plot.title = element_text(family = "serif", face = "bold", size = 13)) 
